...
weather() {
    icon f0c2
    weatherURL='http://www.accuweather.com/en/ar/buenos-aires/7894/weather-forecast/7894'
...
network() {
    ...
    ip link show $enp2s0 | grep 'state UP' >/dev/null && int=$enp2s0 ||int=$wifi
    icon f0ac
    echo $int
...
pacmanUpdates() {
    updates=$(eval pacman -Qu | wc --lines)
    command='termite -e "pacman -Syu"'
...
while :; do
    ...
    [ -z "$*" ] && items="mail pacmanUpdates mpd network volume weather clock themeSwitch" \
                || items="$@"
