...
for i in $num_mon; do
    ...
    cat "$PANEL_FIFO$i" | panel_bar |\
        lemonbar -g $(expr $CUR_MON_WIDTH - $(expr $PANEL_GAP + $PANEL_GAP))x$PANEL_HEIGHT+$(expr $CUR_X_OFFSET + $PANEL_GAP)+$PANEL_GAP -f "$PANEL_FONT_MAIN" -f "$PANEL_FONT_ICON" -f "font\-linux-10" -u 
$PANEL_UNDERLINE | sh &
